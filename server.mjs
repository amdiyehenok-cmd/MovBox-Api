...
const CACHE_MAX = 500;
const cache = new Map();
function keyOf(p) { return [p.detailPath, p.subjectId, p.season||0, p.episode||0].join('|'); }
function cacheGet(k) { const e=cache.get(k); if(!e) return null; if(Date.now()-e.ts>CACHE_TTL_MS){cache.delete(k);return null;} return e.data; }
function cacheSet(k,d) { if(cache.size>=CACHE_MAX){cache.delete(cache.keys().next().value);} cache.set(k,{ts:Date.now(),data:d}); }
...
async function handleSubtitle(req,res,url){
  ...
  res.writeHead(200,{'content-type':ct,'content-length':String(buf.length),'access-control-allow-origin':'*'});
  res.end(buf);
}
